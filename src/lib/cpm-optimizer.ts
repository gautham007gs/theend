...
export class CPMOptimizer {
  ...
  static getOptimalRefreshInterval(placementId: string): number {
    ...
    if (!data) return 60; // Default 60 seconds

    // Higher CTR = faster refresh (more revenue)
    if (data.ctr > 2.0) return 45; // Premium performance
    if (data.ctr > 1.0) return 55; // Good performance
    if (data.ctr > 0.5) return 65; // Average performance
    return 90; // Poor performance - slow down to avoid waste
  }
